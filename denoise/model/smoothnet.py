...
from ..recalls.process_data import cal_diff
...
class SmoothNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.encoder_v = nn.Sequential(
            nn.Linear(window_size-1, hidden_size),
            nn.LeakyReLU(0.1, inplace=True))
        self.encoder_a = nn.Sequential(
            nn.Linear(window_size-2, hidden_size),
            nn.LeakyReLU(0.1, inplace=True))

        def __get_res_blocks():
        ...
        self.res_blocks = __get_res_blocks()
        self.res_blocks_v = __get_res_blocks()
        self.res_blocks_a = __get_res_blocks()

        # Build decoder layers
        self.decoder = nn.Linear(hidden_size, output_size)
        self.decoder_v = nn.Linear(hidden_size, output_size)
        self.decoder_a = nn.Linear(hidden_size, output_size)

        self.fushion_layer = nn.Linear(output_size*3-3, output_size)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = self.res_blocks(x)
        v = self.res_blocks_v(v)
        a = self.res_blocks_a(a)

        x = self.decoder(x)  # [N, C, output_size]
        v = self.decoder_v(v)
        a = self.decoder_a(a)

        x = self.fushion_layer(torch.cat([x,v,a],dim=2))
